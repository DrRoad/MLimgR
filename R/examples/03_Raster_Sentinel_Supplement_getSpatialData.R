...
# Filter Records
  records_filtered <- records %>%
      filter(processinglevel == "Level-1C") %>%
      filter(cloudcoverpercentage < 30)
    # records[which(records$processinglevel == "Level-1C"),] #filter by Level
...
# Preview on Map
  ...
  for(i in 1:nrow(records_filtered[])){
    print(i)
    getSentinel_data(records_filtered[i, ])}
                   ...
                   # .packages='getSpatialData') %dopar% {
                     # getSentinel_data(records_filtered[1, ])
                   # }
